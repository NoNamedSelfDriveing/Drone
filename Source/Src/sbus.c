...
void receive_sbus_packet()
{
    ...
    {  
      /* 스타트 바이트 검사 */
      if((sbus.new_packet_flag) && ((sbus_dma_receive_buff[check_idx] == START_BYTE_1) || (sbus_dma_receive_buff[check_idx] == START_BYTE_2)))
      {
...
{
  static uint16_t prev_throttle = 0, now_throttle = 0;
